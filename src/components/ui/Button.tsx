...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'default', luxury = false, className, children, asChild, href, target = '_blank', rel = 'noopener noreferrer', ...props }, ref) => {
    ...
    const variants = {
      primary: `bg-forest text-white hover:bg-opacity-90 shadow-lg hover:shadow-xl soft-glow ${luxury ? 'bg-gradient-to-r from-forest to-forest/90' : ''}`,
      ...
      secondary: `border-2 border-forest text-forest hover:bg-forest hover:text-white shadow-lg hover:shadow-xl ${luxury ? 'border-gradient' : ''}`,
      ghost: `border-2 border-gold text-gold hover:bg-transparent hover:border-forest hover:text-white backdrop-blur-sm ${luxury ? 'border-gradient' : ''}`
    };
